...
from utils.ptp_utils2 import AttendExciteAttnProcessor, AttentionStore
...
class StableDiffusionNdmPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    ...
    def fn_compute_loss(
        ...
        # Get attention maps: shape (H, W, T)
        aggregate_cross_attention_maps = self.attention_store.aggregate_attention(
            from_where=("up", "mid", "down"), is_cross=True
        )  # shape: (96, 96, 77)

        device = torch.device(device) if device else aggregate_cross_attention_maps.device
        aggregate_cross_attention_maps = aggregate_cross_attention_maps.to(device)

        H, W, T = aggregate_cross_attention_maps.shape
        assert T == 77, f"Expected 77 tokens, got {T}"

        cross_attention_maps = aggregate_cross_attention_maps[:, :, 1:-1]  
        cross_attention_maps = cross_attention_maps * 100.0
    ...
    def fn_ndm(
        ...
        latents = latents.to(device).clone().detach()
        text_embeddings = text_embeddings.to(device).clone().detach()

        mu = torch.zeros_like(latents, device=device, requires_grad=True)
        log_var = torch.zeros_like(latents, device=device, requires_grad=True)
        ...
        for k, v in extra_step_kwargs.items():
            if isinstance(v, torch.Tensor):
                extra_step_kwargs[k] = v.to(device)
        ...
        for iteration in tqdm(range(max_step), desc="Optimizing initial noise"):
            ...
            for i, t in enumerate(timesteps):
                ...
                if use_grad_checkpoint:
                    noise_pred_text = checkpoint.checkpoint(
                        self.unet, optimized_latents, t, text_embeddings[1].unsqueeze(0),
                        use_reentrant=False
                    ).sample
                else:
                    noise_pred_text = self.unet(
                        optimized_latents, t, encoder_hidden_states=text_embeddings[1].unsqueeze(0)
                    ).sample

                self.attention_store.step_end()  

                latent_H = self.attention_store.attn_res[0]
                latent_W = self.attention_store.attn_res[1]
                ...
                if denoising_step_for_loss > 1 and i < denoising_step_for_loss - 1:
                    with torch.no_grad():
                        if use_grad_checkpoint:
                            noise_pred_uncond = checkpoint.checkpoint(
                                self.unet, optimized_latents, t, text_embeddings[0].unsqueeze(0),
                                use_reentrant=False
                            ).sample
                        else:
                            noise_pred_uncond = self.unet(
                                optimized_latents, t, encoder_hidden_states=text_embeddings[0].unsqueeze(0)
                            ).sample
            ...
            joint_loss = sum(joint_loss_list) / len(joint_loss_list) if joint_loss_list else torch.tensor(0.0, device=device)
            cross_attn_loss = max(cross_attn_loss_list) if cross_attn_loss_list else torch.tensor(0.0, device=device)

            history.append({
                'log_var': log_var.clone().detach(),
                'mu': mu.clone().detach(),
                'cross_attn_loss': joint_loss.detach().cpu().item(),  
            })
        ...
        if not optimization_succeed:
            ...
            for item in history:
                if item['cross_attn_loss'] < best_loss:
                    ...
                    best_latents = latents * torch.exp(0.5 * item['log_var'].to(device)) + item['mu'].to(device)
    ...
    def register_attention_control(self):
        ...
        for name, old in self.unet.attn_processors.items():
            ...
            if place is None:                # 非主干层保留原 processor
                procs[name] = old
                continue
    ...
    def __call__(
        ...
        latent_H = height // 8          # 512→64, 768→96, 1024→128
        latent_W = width  // 8          
