...
class StableDiffusionDetectPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    r"""
    ...
    def __call__(
        ...
        # 5. 计算 latent 分辨率 (H=W=图片边长÷8)
        latent_H = height // 8          # 512→64, 768→96, 1024→128
        latent_W = width  // 8          # SD 是方图，两者相等；保留两个名字方便读
        ...
        if not return_dict:
            return (image, has_nsfw_concept, first_latent)
