...
class Attention(nn.Module):
    ...
    def get_attention_scores(
        ...
        """
        dtype = query.dtype #torch.Size([2, 4096, 1536])
        ...
        # --------------------------
        # 关键修改：修正baddbmm_input的形状
        # --------------------------
        if attention_mask is None:
            # 正确形状：[batch, heads, seq_q, seq_k]
            baddbmm_input = torch.empty(
                query.shape[0],  # batch=2
                query.shape[1],  # heads=24
                query.shape[2],  # seq_q=4096
                key.shape[2],    # seq_k=333（注意这里用key的第2维，不是第1维）
                dtype=query.dtype, 
                device=query.device
            )
            beta = 0
        else:
            # 注意：如果有mask，mask形状也必须是[2,24,4096,333]才能匹配
            baddbmm_input = attention_mask
            beta = 1
        ...
        # 批量矩阵乘法：适配多头维度
        # --------------------------
        # key转置最后两维：[2,24,333,64] → [2,24,64,333]
        # 矩阵乘法：query（2,24,4096,64） × key转置（2,24,64,333） → （2,24,4096,333）
        attention_scores = torch.baddbmm(
            baddbmm_input.flatten(0, 1),  # 合并batch和heads到第一维：[2*24, 4096, 333]
            query.flatten(0, 1),          # 合并后：[2*24, 4096, 64]
            key.transpose(-1, -2).flatten(0, 1),  # 合并后：[2*24, 64, 333]
            beta=beta,
            alpha=self.scale,
        ).view_as(baddbmm_input)  # 恢复多头形状：[2,24,4096,333]
        del baddbmm_input
        ...
        # print(attention_probs.shape) # 输出：[2,24,4096,333]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is not None:
            ...
            record_key = encoder_hidden_states_key_proj 
            record_query2 = encoder_hidden_states_query_proj 
            
            query = torch.cat([query, encoder_hidden_states_query_proj], dim=2) #[2, 24, 4429, 64]
        ...
        # o_x = hidden_states #new-ox
        # o_c = encoder_hidden_states #new-oc
        # o_c_T = o_c.transpose(1, 2)  # 调整后：[3, 1536, 333]
        ...
        if encoder_hidden_states is not None:
            return hidden_states, encoder_hidden_states, record_query, record_key, record_query2#, o_x, o_c_T #output
        else:
            return hidden_states
...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if encoder_hidden_states is not None:
            ...
            record_key = encoder_hidden_states_key_proj 
            record_query2 = encoder_hidden_states_query_proj 
            # print("query shape:",encoder_hidden_states_query_proj.shape, query.shape)
            # attention
            query = torch.cat([encoder_hidden_states_query_proj, query], dim=2)
            key = torch.cat([encoder_hidden_states_key_proj, key], dim=2)
            value = torch.cat([encoder_hidden_states_value_proj, value], dim=2)

            # print("query new shape:", query.shape)
        ...
        if encoder_hidden_states is not None:
            ...
            return hidden_states, encoder_hidden_states, record_query, record_key, record_query2
        else:
            return hidden_states
